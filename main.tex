...
This is exactly, the problem we are trying to solve in this thesis. Instead of using a fixed set of well-chosen random projection vectors to compute the Sliced-Wasserstein distance as in \cite{LuanHamp2025}, we propose to use the UnifOrtho method designed by \cite{RowlandEtAl2019} to sample orthogonal projections, which leads to a significant reduction in variance and more stable clustering results in high-dimensions.
...
Regime detection in financial markets is heavily studied topic in literature. 
...
\begin{equation}
    \text{Hit Ratio} = \frac{\text{Number of profitable trades}}{\text{Total number of trades}} \times 100
\end{equation}
